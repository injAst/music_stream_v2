    ...
    return Scaffold(
      ...
      body: Consumer<LibraryController>(
        builder: (context, lib, _) {
          ...
          var newReleases = tracks
              .where(
                (t) => t.createdAt != null && t.createdAt!.isAfter(weekAgo),
              )
              .toList();

          // Если за неделю ничего не вышло — показываем последние добавленные (сортируем по дате)
          if (newReleases.isEmpty) {
            newReleases = List<Track>.from(tracks)
              ..sort((a, b) {
          ...
          final trendingTracks = List<Track>.from(tracks)
            ..sort((a, b) {
          ...
          final trendingAll = trendingTracks
              .take(100)
              .toList(); // Вернули лимит в 100 треков
          final trendingHome = trendingTracks
              .take(8)
              .toList(); // Вернули 8 на главном экране
          ...
          return CustomScrollView(
            ...
            slivers: [
              ...
              SliverToBoxAdapter(child: _FlowHero(tracks: tracks)),
                    ...
                    child: ListView.separated(
                      ...
                      itemBuilder: (context, i) {
                        return _TrendingTrackCard(
                          ...
                          onTap: () =>
                              context.read<AudioPlayerController>().playTrack(
                                trendingHome[i],
                                playlist: trendingHome,
                              ),
                        );
                      },
                    ),
                    ...
                    itemBuilder: (context, i) {
                      ...
                      return _VerticalTrackTile(
                        ...
                        onPlay: () => context
                            ...
                            .playTrack(t, playlist: recommendationsHome),
          ...
          child: Column(
            ...
            children: [
              ...
              ListTile(
                ...
                title: Text(
                  track.isLiked ? 'Удалить из любимых' : 'Добавить в любимые',
                ),
                onTap: () {
                  Navigator.pop(context);
...
class _FlowHeroState extends State<_FlowHero> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    _themeTransitionController = AnimationController(
      ...
      duration: const Duration(
        seconds: 8,
      ), // Очень плавный переход цвета (8 сек)
    )..value = 1.0;
    ...
    _cycleTimer = Timer.periodic(const Duration(seconds: 45), (timer) {
      if (mounted) {
        setState(() {
          ...
          _currentPattern =
              _FlowPatternType.values[(_currentPattern.index + 1) %
                  _FlowPatternType.values.length];
            ...
            Positioned.fill(
              child: AnimatedBuilder(
                animation: Listenable.merge([
                  ...
                  _patternTransitionController,
                ]),
                builder: (context, child) {
                  final lerpTheme = _themeTransitionController.value;
                  final prevThemeIndex =
                      (_currentThemeIndex - 1 + _palettes.length) %
                      _palettes.length;
                  final currentColors = List.generate(
                    4,
                    (i) => Color.lerp(
                      _palettes[prevThemeIndex][i],
                      _palettes[_currentThemeIndex][i],
                      lerpTheme,
                    )!,
                  );
            ...
            Padding(
              ...
              child: Row(
                children: [
                  ...
                  _NebulaPlayButton(
                    isPlaying: isPlaying,
                    onTap: () => context.read<AudioPlayerController>().playWave(
                      widget.tracks,
                    ),
                  ),
                ],
  ...
  Widget _buildPatternPainter(
    _FlowPatternType pattern,
    List<Color> colors,
    bool isPlaying,
  ) {
    switch (pattern) {
...
class _NebulaPlayButtonState extends State<_NebulaPlayButton>
...
class _NebulaBackgroundPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    void drawCloud(
      ...
      final centerX =
          size.width / 2 +
          size.width * 0.35 * math.cos(val * speed + phase) * xMult;
      final centerY =
          size.height / 2 +
          size.height * 0.35 * math.sin(val * speed + phase) * yMult;

      canvas.drawCircle(
        Offset(centerX, centerY),
        size.width * 0.4 * radiusMult,
        paint,
      );
    }
...
class _AuroraBackgroundPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < 4; i++) {
      final paint = Paint()
        ..color = colors[i]
            .withValues(alpha: 0.7) // Увеличили непрозрачность
        ..strokeWidth =
            140 // Сделали ленты шире
        ..style = PaintingStyle.stroke
        ..blendMode = BlendMode
            .screen // Добавили свечение
        ..maskFilter = const MaskFilter.blur(
          BlurStyle.normal,
          60,
        ); // Уменьшили размытие для четкости
      ...
      for (double y = 0; y < size.height + 200; y += 30) {
        final xOffset =
            80 * math.sin(y / size.height * math.pi + val * speed + i * 2);
...
class _WavesBackgroundPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < 4; i++) {
      final paint = Paint()
        ..color = colors[i]
            .withValues(alpha: 0.6) // Ярче
        ..blendMode = BlendMode
            .plus // Слой на слой для яркости
      ...
      for (double x = 0; x < size.width + 50; x += 20) {
        final y =
            yBase +
            40 * math.sin(x / size.width * 2 * math.pi + val * speed + i * 3);
        path.lineTo(x, y);
      }
...
class _StardustBackgroundPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < 3; i++) {
      ...
      final x =
          (size.width * 0.3) + (size.width * 0.4 * math.sin(val * 0.5 + i));
    ...
    for (int i = 0; i < 80; i++) {
      ...
      final xShift =
          (animationValue * speed * 100 + i * 20) % (size.width + 100) - 50;
      ...
      canvas.drawCircle(
        Offset(
          (basePosX + xShift) % size.width,
          (basePosY + yShift) % size.height,
        ),
...
void _drawStars(Canvas canvas, Size size, double val) {
  ...
  for (int i = 0; i < 30; i++) {
    ...
    canvas.drawCircle(
      Offset(x + orbit, y + orbit),
      0.7 + rand.nextDouble() * 0.5,
      starPaint,
    );
  }
}
    ...
    return Padding(
      ...
      child: GridView.builder(
        ...
        itemBuilder: (context, i) {
          final t = items[i];
          return GestureDetector(
            onTap: () => context.read<AudioPlayerController>().playTrack(
              t,
              playlist: tracks,
            ),
            child: Column(
                            ...
                            (u) => ListTile(
                              leading: CircleAvatar(
                                ...
                                backgroundImage: u.avatarUrl != null
                                    ? NetworkImage(
                                        ApiConfig.resolveUrl(u.avatarUrl)!,
                                      )
                                    : null,
